...
import { getPostsByTitle } from '../../graphql/queries';
...
export default function Category() {
  ...
  const [getPosts, { data }] = useLazyQuery(getPostsByTitle);
  ...
  const previousPosts = () => {
    ...
    if (newValue <= 0) {
      router.push(`/pesquisa/${path}?pagina=1`);
      setCurrentPage(1);
    } else {
      router.push(`/pesquisa/${path}?pagina=${newValue}`);
  ...
  const nextPosts = () => {
    ...
    if (newValue > data?.blogPosts?.meta?.pagination?.pageCount) {
      router.push(
        `/pesquisa/${path}?pagina=${data?.blogPosts?.meta?.pagination?.pageCount}`
      );
  ...
  const goToPage = (e) => {
    ...
    if (currentPage !== value) {
      router.push(`/pesquisa/${path}?pagina=${value}`);
